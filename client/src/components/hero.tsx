        ...
        <div className="mx-auto w-80 h-80 mb-8 coin-shadow float-animation">
          {!fallback ? (
            <img
              ...
              onError={() => {
                console.error('FTFC image failed to load, falling back to logo');
                setFallback(true);
              }}
            />
          ) : (
            <img
              src="/logo.svg"
              alt="FTFC Logo Fallback"
              className="w-full h-full object-contain"
              data-testid="hero-logo-fallback"
            />
          )}
        </div>

        <h1 className="font-urban text-6xl md:text-8xl font-bold mb-4 tracking-tight">
          <span className="text-ftf-gold text-outline-black" data-testid="hero-title-line1">FUCK THE</span><br />
          <span className="text-ftf-gold text-outline-black-bold" data-testid="hero-title-line2">FARMERS COIN</span>
        </h1>
